...
public class DUSContracts {
    ...
    public static Uri buildFetchUpdateGraphUriWithError(String result) {
        return buildFetchUpdateGraphUri().buildUpon().appendQueryParameter(ERROR, result).build();
    }
    ...
    @NonNull
    public static String getComponentsQuery(int size) {
        return COMPONENT_KEY + " IN (" + makePlaceholders(size) + ")";
    }
